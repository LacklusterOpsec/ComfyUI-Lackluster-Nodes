...
export class CameraWidget {
      ...
      for (const h of handles) {
        if (this.raycaster.intersectObject(h.mesh).length > 0) {
          ...
          if (this.hoveredHandle !== h) {
            if (this.hoveredHandle) this.setHandleScale(this.hoveredHandle.mesh, this.hoveredHandle.glow, 1.0)
            this.hoveredHandle = h
            this.setHandleScale(h.mesh, h.glow, 1.2)
  ...
  public generatePrompt(): string {
    return buildAnglePrompt(this.state.azimuth, this.state.elevation, this.state.distance)
  }
...
export function buildAnglePrompt(azimuth: number, elevation: number, distanceVal: number): string {
  const hAngle = ((azimuth % 360) + 360) % 360
